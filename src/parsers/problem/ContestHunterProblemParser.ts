...
export class ContestHunterProblemParser extends Parser {
  ...
  public async parse(url: string, html: string): Promise<Sendable> {
    ...
    if (params1.length !== 0) {
      ...
      task.setTimeLimit(Math.round(parseFloat(/([0-9.]+)ms/.exec(limitsStr)[1])));
      task.setMemoryLimit(Math.round(parseFloat(/([0-9.]+)kB/.exec(limitsStr)[1]) / 1024));
    } else if (params2.length !== 0) {
      const limitsStr = params2[0].parentElement.textContent;
      task.setTimeLimit(Math.round(parseFloat(/([0-9.]+) s/.exec(limitsStr)[1]) * 1000));
      task.setMemoryLimit(Math.round(parseFloat(/([0-9.]+) MiB/.exec(limitsStr)[1])));
    }
