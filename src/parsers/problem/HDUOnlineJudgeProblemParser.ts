...
export class HDUOnlineJudgeProblemParser extends Parser {
  ...
  public parse(url: string, html: string): Promise<Sendable> {
    return new Promise(resolve => {
      ...
      const contentElem =
        elem.querySelector('tr:nth-child(4)') ||
        elem.querySelector('#contest_nav + div');
      ...
      task.setMemoryLimit(
        Math.floor(parseInt(/Memory Limit: (\d+)/.exec(html)[1], 10) / 1000),
      );
      ...
      const inputs = [...contentElem.querySelectorAll('div')]
        ...
        .map(pre => {
          ...
          return [...dataElem.childNodes].reduce(
