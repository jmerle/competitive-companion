...
export class LanqiaoProblemParser extends Parser {
  ...
  public async parse(url: string, html: string): Promise<Sendable> {
    ...
    const title = document.querySelector('.course-name').textContent;
    task.setName(title.replace(/\s+/g, ' ').trim());

    const limitStr = document.querySelector('#运行限制 + table > tbody > tr').textContent;
    ...
    const input = document.querySelector('#输入样例 + pre > code, #样例输入 + pre > code').textContent;
    const output = document.querySelector('#输出样例 + pre > code, #样例输出 + pre > code').textContent;
